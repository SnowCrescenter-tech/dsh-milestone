...
- **折叠工具栏** —— 顶部功能键默认收起成一行（展开箭头 + 设置齿轮），点箭头展开全部功能，长对话时界面更清爽。
- **自定义快捷区** —— 设置菜单里把常用功能（搜索 / 列表 / 收藏筛选等）「钉」在折叠外，一键直达；偏好持久保存，一键恢复默认。
- **更新检测** —— 自动检查 npm 新版本（6 小时缓存，不打扰），发现新版按钮亮起提醒徽章；点开看当前/最新版本与来源，一键去 npm 升级；弹窗内展示插件**已适配的官方版本线**。
- **点击外部关闭** —— 搜索 / 列表 / 跨会话搜索等浮层，点面板外的任意位置即自动收起（与官方 capture 阶段 pointerdown 契约一致）。
- **设置底部推广区** —— 设置菜单底部直达 GitHub 仓库、欢迎 Star、提交 Issue，以及 npm 安装渠道。
...
- 更新检测弹窗内会展示本插件声明支持的版本线；harness 当前版本在浏览器端没有可信来源（`host.describe().version` 是占位值），因此不做精确探测，以声明线为准。
...
- 更新检测依赖浏览器能访问 npm 镜像（`registry.npmmirror.com` / `registry.npmjs.org`）；若页面 CSP 限制 `connect-src` 或离线，检查会静默失败，不打扰使用。结果缓存 6 小时。
